...
class CloudScale_DevTools {
    ...
    public static function init() {
        ...
        // Reject malformed Basic Auth (empty username, oversized, invalid chars) with 400.
        add_filter( 'rest_authentication_errors', [ 'CSDT_Login', 'rest_reject_empty_basic_auth' ], 1 );
...
<div style="margin-top:14px;display:flex;align-items:center;gap:10px">
    ...
    private static function render_login_panel(): void {
        ...
        <div class="cs-panel" id="cs-panel-hide-login">
            ...
            <div class="cs-panel-body">
                ...
                <?php
                $masked_login_url = str_repeat( '•', 32 );
                ?>
    ...
    public static function render_dashboard_widget(): void {
        ...
        // Deduplicated failed-login entries for the widget: one row per IP (most recent),
        // with today's count per IP. Sorted newest first, capped at 10 unique IPs.
        $bf_log_reversed  = array_reverse( $bf_log );
        $bf_seen_ips      = [];
        $bf_ip_today      = [];
        $bf_widget_rows   = [];
        foreach ( $bf_log_reversed as $entry ) {
            $ts  = (int) ( $entry[0] ?? $entry['time'] ?? 0 );
            $usr = (string) ( $entry[1] ?? $entry['username'] ?? '' );
            $ip  = (string) ( $entry[2] ?? $entry['ip'] ?? '' );
            $cc  = (string) ( $entry[3] ?? $entry['country'] ?? '' );
            if ( gmdate( 'Y-m-d', $ts ) === $today_str ) {
                $bf_ip_today[ $ip ] = ( $bf_ip_today[ $ip ] ?? 0 ) + 1;
            }
            if ( ! isset( $bf_seen_ips[ $ip ] ) ) {
                $bf_seen_ips[ $ip ] = true;
                $bf_widget_rows[]   = [ 'time' => $ts, 'user' => $usr, 'ip' => $ip, 'cc' => $cc ];
            }
        ...
        // Pre-fetch all users in two batch queries instead of one per unique username.
        $all_bf_names = [];
        foreach ( $bf_log as $e ) {
            $n = (string) ( $e[1] ?? $e['username'] ?? '' );
            if ( $n ) { $all_bf_names[ $n ] = true; }
        }
        $all_bf_names = array_keys( $all_bf_names );

        $bf_user_map = []; // username => WP_User|false
        if ( $all_bf_names ) {
            foreach ( get_users( [ 'login__in' => $all_bf_names, 'number' => -1 ] ) as $found_u ) {
                $bf_user_map[ $found_u->user_login ] = $found_u;
            }
            $need_email = array_values( array_filter( $all_bf_names, fn( $n ) => ! isset( $bf_user_map[ $n ] ) ) );
            if ( $need_email ) {
                global $wpdb;
                $fmt = implode( ',', array_fill( 0, count( $need_email ), '%s' ) );
                // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery
                foreach ( (array) $wpdb->get_results( $wpdb->prepare( "SELECT ID, user_email FROM {$wpdb->users} WHERE user_email IN ({$fmt})", $need_email ) ) as $row ) {
                    $bf_user_map[ $row->user_email ] = get_userdata( $row->ID );
                }
            }
            foreach ( $all_bf_names as $n ) {
                if ( ! isset( $bf_user_map[ $n ] ) ) { $bf_user_map[ $n ] = false; }
            }
        }
        ...
        <!-- ── API path attack urgent warning ───────────────────────── -->
        <?php if ( $api_attacks_today > 0 ) : ?>
        <a href="<?php echo esc_url( admin_url( 'tools.php?page=cloudscale-devtools&tab=credentials' ) ); ?>"
           style="display:flex;align-items:center;gap:8px;background:#7f1d1d;border-bottom:2px solid #ef4444;padding:8px 12px;margin:-1px -12px 10px;text-decoration:none;cursor:pointer;">
        ...
        <div class="cs-dw-grid">
            ...
            <div class="cs-dw-chip">
                ...
                <span>
                    ...
                    <?php echo ! empty( $login_slug ) ? esc_html__( 'Active', 'cloudscale-devtools' ) : esc_html__( 'Off', 'cloudscale-devtools' ); ?>
                </span>
            </div>
            <div class="cs-dw-chip">
        ...
        <div style="display:flex;flex-direction:column;gap:5px;margin-bottom:12px;">
            <a href="<?php echo esc_url( $base_url . '&tab=login' ); ?>" style="display:flex;align-items:flex-start;gap:7px;flex-shrink:0;background:#fff7ed;border:1px solid #fdba74;border-radius:6px;padding:6px 9px;text-decoration:none;cursor:pointer;" title="<?php esc_attr_e( 'Go to Login Security', 'cloudscale-devtools' ); ?>">
                ...
                <div style="flex:1;min-width:0">
                    ...
                    <div style="font-size:10px;color:#64748b;margin-top:1px;">
                        <?php
                        if ( $latest_usr ) {
                            $ulen       = mb_strlen( $latest_usr );
                            $show_start = min( 2, $ulen );
                            $show_end   = $ulen > 4 ? 2 : 0;
                            $mask_len   = max( 0, $ulen - $show_start - $show_end );
                            $masked_usr = mb_substr( $latest_usr, 0, $show_start )
                                        . str_repeat( '•', $mask_len )
                                        . ( $show_end ? mb_substr( $latest_usr, -$show_end ) : '' );
                        } else {
                            $masked_usr = '—';
                        }
                        ?>
                        <?php esc_html_e( 'Username:', 'cloudscale-devtools' ); ?> <?php echo esc_html( $masked_usr ); ?> &nbsp;&middot;&nbsp;
            ...
            <div style="margin-top:6px;">
                ...
                <div style="display:none;margin-top:6px;background:#fef2f2;border:1px solid #fca5a5;border-radius:6px;overflow:hidden;">
                    <?php foreach ( $targeted_users as $tuser ) : ?>
                    <div style="display:flex;align-items:center;gap:8px;padding:7px 10px;border-bottom:1px solid rgba(252,165,165,.4);">
                        <?php if ( $tuser['deleted'] ) : ?>
                        <span style="font-size:14px;flex-shrink:0;">⚫</span>
                        <span style="font-size:12px;font-weight:700;color:#6b7280;flex:1;"><?php echo esc_html( $tuser['login'] ); ?></span>
                        <span style="font-size:10px;padding:2px 6px;border-radius:6px;background:#f1f5f9;color:#94a3b8;border:1px solid #e2e8f0;white-space:nowrap;"><?php esc_html_e( 'deleted', 'cloudscale-devtools' ); ?></span>
                        <?php else : ?>
                        <span style="font-size:14px;flex-shrink:0;">🔴</span>
                        <span style="font-size:12px;font-weight:700;color:#991b1b;flex:1;"><?php echo esc_html( $tuser['login'] ); ?></span>
                        <span style="font-size:10px;color:#94a3b8;white-space:nowrap;"><?php echo esc_html( $tuser['email_mask'] ); ?></span>
                        <?php endif; ?>
                    </div>
                    <?php endforeach; ?>
                    <?php if ( $invalid_count > 0 ) : ?>
                    <div style="padding:6px 10px;font-size:11px;color:#6b7280;background:#fff8f8;">
                        + <?php echo esc_html( $invalid_count ); ?> <?php esc_html_e( 'invalid username attempts (bots guessing)', 'cloudscale-devtools' ); ?>
                    </div>
                    <?php endif; ?>
                    <div style="padding:5px 10px;font-size:10px;color:#94a3b8;background:#fff8f8;"><?php esc_html_e( 'Real accounts targeted — consider changing their passwords.', 'cloudscale-devtools' ); ?></div>
                </div>
            </div>
            <?php endif; ?>
        </div>
        ...
        <?php
        $raw_events = get_option( 'csdt_security_events', [] );
        $raw_events = is_array( $raw_events ) ? array_reverse( $raw_events ) : [];

        // Deduplicate: keep only the latest event per type, track today's count per type.
        $today_str  = gmdate( 'Y-m-d' );
        $seen_types = [];
        $today_counts = [];
        $deduped    = [];
        ...
        // Cap at 10 unique types.
        $deduped = array_slice( $deduped, 0, 10 );

        if ( ! empty( $deduped ) ) :
        ?>
        <div class="cs-dw-section">⚠️ <?php esc_html_e( 'Recent Security Events', 'cloudscale-devtools' ); ?></div>
        <div style="display:flex;flex-direction:column;gap:5px;margin-bottom:12px;max-height:260px;overflow-y:auto;padding-right:2px;">
            <?php foreach ( $deduped as $ev ) :
